...
export default function ViajeScreen({ navigation }) {
  ...
  const guardarViaje = async (datoViaje) => {
    await axios.put(urlApi + urlViaje + '/' + datoViaje.id, datoViaje)
      .then((re) => {
  ...
  const revisarMarked = (rutaIV,loc) => {
    if (!rutaIV.origin.marked
      && loc.latitude.toFixed(3) === rutaIV.origin.location.lat.toFixed(3)
      && loc.longitude.toFixed(3) === rutaIV.origin.location.lng.toFixed(3)
      && new Date() > new Date(new Date(rutaIV.origin.time)
        .setMinutes(new Date(rutaIV.origin.time)
          .getMinutes() - 15))
      && new Date() < new Date(new Date(rutaIV.origin.time)
        .setMinutes(new Date(rutaIV.origin.time)
          .getMinutes() + 15))
      ...
      console.log('se guarda ida origen')
      rutaIV.origin.marked = new Date(loc.timestamp)
      //console.log(viaje.ruta.ida.origin.marked)
      guardarViaje(viaje)
    }

    rutaIV.waypoints = rutaIV.waypoints.map(e => {
      //console.log(e)
      if (!e.marked
    ...
    if (!rutaIV.destination.marked
      && loc.latitude.toFixed(3) === rutaIV.destination.location.lat.toFixed(3)
      && loc.longitude.toFixed(3) === rutaIV.destination.location.lng.toFixed(3)
      && new Date() > new Date(new Date(rutaIV.destination.time)
        .setMinutes(new Date(rutaIV.destination.time)
          .getMinutes() - 15))
      && new Date() < new Date(new Date(rutaIV.destination.time)
        .setMinutes(new Date(rutaIV.destination.time)
          .getMinutes() + 15))
    ) {
      rutaIV.destination.marked = new Date(loc.timestamp)
  ...
  const saveLocation = async (loc) => {
    ...
    if (viaje) {
      revisarMarked(viaje.ruta.ida,loc)
      revisarMarked(viaje.ruta.vuelta,loc)
      /*console.log('----------------------')
      ...
      //console.log(!viaje.ruta.ida.origin.marked)
      //console.log(loc.latitude.toFixed(3) === viaje.ruta.ida.origin.location.lat.toFixed(3))
      //console.log(loc.longitude.toFixed(3) === viaje.ruta.ida.origin.location.lng.toFixed(3))
      /*console.log(new Date() > new Date(new Date(viaje.ruta.ida.origin.time)
      .setMinutes(new Date(viaje.ruta.ida.origin.time)
        .getMinutes() - 5)))*/
